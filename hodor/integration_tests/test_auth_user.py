...
class TestJWT:
    def test_register(self, config_valid, user_valid1):
        _response = requests.post(url=f'{config_valid["root_url"]}/api/signup',
    ...
    def test_profile(self, config_valid, admin_access_token):
        _response = requests.get(url=f'{config_valid["root_url"]}/api/profile',
    ...
    def test_logout(self, config_valid, admin_access_token_logout):
        _response = requests.get(url=f'{config_valid["root_url"]}/api/signout',
                                 headers={'Content-Type': 'application/json',
    ...
    def test_jwk(self, config_valid):
        _response = requests.get(url=f'{config_valid["root_url"]}/api/.well-known/jwks.json',
                                 headers={'Content-Type': 'application/json'},
                                 timeout=3)
        assert _response.status_code == 200
